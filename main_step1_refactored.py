...
# (import 群のどこかで)
from .geometry import create_shape      # 追加
...
def _safe_anim_save(anim, output_path):                  
    """
    ffmpeg が無くても PillowWriter で保存を試みる安全ラッパー
    """
    try:
        anim.save(output_path, writer="ffmpeg", codec="mpeg4", fps=5)
    except Exception as e:
        print(f"[WARN] ffmpeg 保存失敗 ({e}) → pillow writer で再試行")
        try:
            anim.save(output_path, writer="pillow", fps=5)
        except Exception as e2:
            print(f"[ERROR] pillow writer でも保存に失敗しました: {e2}")
...
def get_constants_from_gui(selected_data, shape, volume, sperm_conc):
    ...
    constants['initial_stick_status'] = int(selected_data.get('initial_stick_status', 0))
    ...
    if shape == "cube":
        ...
        positions_map = {
            "center": (
                0,
                0,
                (constants['spot_bottom_height'] + constants['spot_R']) / 2
            ),
            "bottom_center": (
                0,
                0,
                constants['spot_bottom_height'] + gamete_R
            ),
            "bottom_edge": (
                np.sqrt(
                    (constants['spot_R'] - gamete_R)**2
                    - (constants['spot_bottom_height'] + gamete_R)**2
                ),
                0,
                constants['spot_bottom_height'] + gamete_R
            )
...
class SpermSimulation:
    def __init__(self, constants, visualizer, simulation_data):
        ...
        if constants.get('reflection_analysis', 'no') == "yes":
            self.initial_stick_status = constants['initial_stick_status']
        else:
            self.initial_stick_status = 0
    ...
    def initial_vec(self, j, constants):
        ...
        if reflection_mode == "yes":
            ...
            if shape == "cube" or shape == "ceros":
                ...
                IO_status = IO_check_drop(temp_position, self.constants['initial_stick_status'], self.constants)
                vertex_point = None
            elif shape == "spot":
                IO_status = IO_check_spot(base_position, temp_position, self.constants, "none")
                vertex_point = None
            local_stick = self.constants['initial_stick_status']
        ...
        elif (reflection_mode == "no") and (analysis_type == "single_simulation"):
            if shape in ["cube", "ceros"]:
                ...
                base_position = np.array([x_base, y_base, z_base])
            elif shape == "drop":
    ...
    def single_sperm_simulation(self, j, base_position, temp_position, remaining_distance, constants):
        if constants['reflection_analysis'] == "yes":
            stick_status = self.initial_stick_status
        else:
            stick_status = 0
        self.trajectory[j, 0] = base_position
        i = 1
...
class SpermPlot:
    ...
    def __init__(self, simulation):
        ...
        self.x_min = self.constants.get('x_min')
        self.x_max = self.constants.get('x_max')
        self.y_min = self.constants.get('y_min')
        self.y_max = self.constants.get('y_max')
        self.z_min = self.constants.get('z_min')
        self.z_max = self.constants.get('z_max')
        if None in (self.x_min, self.x_max,
    ...
    def set_min_max(self, volume):
        ...
        if shape == "cube":
            edge = volume ** (1 / 3)
            half_edge = edge / 2
            self.x_min = self.y_min = self.z_min = -half_edge
            self.x_max = self.y_max = self.z_max =  half_edge
        elif shape == "spot":
            r  = self.constants['radius']
            bh = self.constants['spot_bottom_height']
            self.x_min = self.y_min = -r
            self.x_max = self.y_max =  r
            self.z_min = bh
            self.z_max = r
        elif shape == "drop":
            r  = self.constants['drop_R']
            self.x_min = self.y_min = self.z_min = -r
            self.x_max = self.y_max = self.z_max =  r
        elif shape == "ceros":
            self.x_min, self.x_max = -8.15,  8.15
            self.y_min, self.y_max = -6.20,  6.20
            self.z_min, self.z_max = -0.05,  0.05
    ...
    def set_ax_3D(self, ax):
        ...
        if shape == "cube":
            ...
            r  = self.constants['radius']
            bh = self.constants['spot_bottom_height']
            ax.set_xlim(-r, r); ax.set_ylim(-r, r); ax.set_zlim(bh, r)
            ax.set_box_aspect([2*r, 2*r, r-bh])
        elif shape == "drop":
            r = self.constants['drop_R']
            ...
            ax.set_box_aspect([1.63, 1.24, 0.1])
    def _draw_graph(self, shape):
        ...
        pbar = tqdm(total=self.simulation.number_of_sperm * (self.simulation.n_simulation - 1) * len(axis_combi),
                    desc="Plotting trajectories", ncols=100, ascii=True)
        for j in range(self.simulation.number_of_sperm):
            for i in range(self.simulation.n_simulation - 1):
        ...
        pbar.close()
        if shape == "spot":
            rb = self.constants['spot_bottom_R']
            bh = self.constants['spot_bottom_height']
            sr = self.constants['spot_R']
            axes[0].set_xlim(-rb, rb); axes[0].set_ylim(-rb, rb)
            axes[1].set_xlim(-rb, rb); axes[1].set_ylim(bh, sr)
            axes[2].set_xlim(-rb, rb); axes[2].set_ylim(bh, sr)
            for ax in axes:                                           
            ...
            ax_single.set_xlim(-0.815, 0.815)
            ax_single.set_ylim(-0.62 , 0.62 )
...
def load_previous_selection():
    ...
    initial_stick_status_default = get_str("initial_stick_status",      "10")
    ...
    return {
        ...
        "initial_stick_status": initial_stick_status_default,
...
def save_previous_selection(values):
    ...
    single_keys = [
        ...
        "gamete_r", "initial_direction", "initial_stick_status", "analysis_type"
    ]
...
def show_selection_ui(no_gui=False):
    ...
    radio_button_defaults = {
        ...
        "initial_stick_status_options":   ["0", "1", "10", "20"],
    }
    ordered_keys = [
        ...
        ("初期Stick状態 (Initial Stick)",      "initial_stick_status_options"),
    ]
    def update_states():
        ...
        for key in ["initial_direction_options", "initial_stick_status_options"]:
            if key in frames:
...
def main():
    ...
    initial_stick_status = int(selected_data.get('initial_stick_status', 10))
    ...
    for shape in shapes_list:
        for volume in volumes_list:
            for sperm_conc in sperm_conc_list:
                ...
                # ---- シミュレーション実行 ----------------------------
                from core.simulation import SpermSimulation          # ★ 追加
